...
public class SchematicPrinter {
    ...
    public boolean print(final WorldClient world, final EntityPlayerSP player) {
		...
		BlockPos centralBlock = null; 
		RayTraceResult hover = minecraft.objectMouseOver; 
		if(hover.typeOfHit == Type.BLOCK) {
			centralBlock = hover.getBlockPos();
		}  else {
			return false;
		}
		EnumFacing blockSide = hover.sideHit;
...
//		final Vec3i relativePlPos = new Vec3i(playerX, playerY, playerZ);
...
//		final double blockReachDistanceSq = blockReachDistance * blockReachDistance;
    ...
    private boolean blockIsReachable(final BlockPos blockToPlace, double playerX, double
    		playerY, double playerZ, final double reachDistance) {
...
//		double playerZ = playerPos.getZ();
		...
		final double reachDistanceSq = reachDistance*reachDistance;
		// TODO why: Math.pow(reachDistance, 2)
		if(blockToPlace.distanceSqToCenter(playerX, playerY, playerZ) <= reachDistanceSq) {
			return true;
		} else {
			return false;
		}
