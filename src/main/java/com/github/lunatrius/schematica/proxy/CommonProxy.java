...
public abstract class CommonProxy {
			...
			for (int chunkLocalZ = localMinZ; chunkLocalZ <= localMaxZ; chunkLocalZ++) {
				for (int y = minY; y <= maxY; y++) {
					...
					try {
						...
						final IBlockState blockState = schematic.getRequiredBlockState(pos, world);
	...
	private void saveSchematic(String iconName, int width, int height, int length,
			EntityPlayer player, World world, File directory,
			String format, int minX, int maxX, int minY, int maxY, int minZ, int maxZ) {
		ISchematic schematic = new Schematic(SchematicUtil.getIconFromName(iconName), 
				width, height, length);

		final SchematicContainer container = new SchematicContainer(schematic, player, world, directory, format, minX, maxX, minY, maxY, minZ, maxZ);
		QueueTickHandler.INSTANCE.queueSchematic(container);
	}

	private void saveGeneratedScheme(Map<BlockPos, IBlockState> blockStates, 
			String iconName, int width, int height, int length,
			EntityPlayer player, World world, File directory,
			String format, int minX, int maxX, int minY, int maxY, int minZ, int maxZ
			) {
		ISchematic schematic = new GeneratedScheme(blockStates,
				SchematicUtil.getIconFromName(iconName), width, height, length);
	...
	public boolean saveSchematic(final EntityPlayer player, final File directory, 
			String filename, final World world, @Nullable final String format, 
			final BlockPos from, final BlockPos to, Map<BlockPos, IBlockState> blockStates) {
		try {
			String iconName = "";
			...
			if(blockStates != null)
				saveGeneratedScheme(blockStates, iconName, width, height, length, player, 
						world, fileDir, format, minX, maxX, minY, maxY, minZ, maxZ);
			else
				saveSchematic(iconName, width, height, length, player, 
						world, fileDir, format, minX, maxX, minY, maxY, minZ, maxZ);
	...
	public boolean saveSchematic(final EntityPlayer player, final File directory, String filename, 
			final World world, @Nullable final String format, final BlockPos from, final BlockPos to) {
		try {
			String iconName = "";
	...
	public abstract boolean loadSchematicFromFile(final EntityPlayer player, final File directory, final String filename);
	...
	public abstract boolean loadSchematic(EntityPlayer player, ISchematic schematic);
	public abstract void loadSchematicFromController(EntityPlayer player);
	public abstract void openGuiSave();
